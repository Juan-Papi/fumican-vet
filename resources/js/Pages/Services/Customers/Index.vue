...
function applyFilters() {
    router.get(route("customers.search"), filters.value, {
        preserveState: true,
        replace: true,
    });
}
...
function openEditModal(customer) {
    ...
    form.value = { ...defaultFormState, ...customer };
    formErrors.value = {};
    isCreateOrEditModal.value = true;
}
...
async function submitForm() {
    ...
    try {
        if (modalMode.value === "edit") {
            await axios.put(
                route("customers.update", selectedCustomer.value.id),
                form.value
            );
            displayToast("success", "Cliente actualizado correctamente.");
        } else {
            await axios.post(route("customers.store"), form.value);
...
async function submitDelete() {
    ...
    try {
        await axios.delete(
            route("customers.destroy", selectedCustomer.value.id)
        );
