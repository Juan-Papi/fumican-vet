...
watch(currentPage, (page) => {
    router.get(route("warehouse.index"), { page }, { preserveState: true });
});
...
async function submitCreate() {
    ...
    loading.value = true;
    try {
        const { data } = await axios.post(route("warehouse.store"), form.value);
                ...
                <FwbTableRow v-for="w in warehouses.data" :key="w.id">
                    ...
                    <FwbTableCell class="text-right">
                        <div class="inline-flex space-x-1">
                            <FwbA
                                :href="route('warehouse.show', w.id)"
                                class="p-1 rounded hover:bg-gray-100"
                            >
                                <i class="fa-solid fa-eye text-black"></i>
                            </FwbA>
                            <FwbA
                                @click.prevent="openEditModal(w)"
                                class="p-1 rounded hover:bg-gray-100"
                            >
                                <i class="fa-solid fa-pencil text-black"></i>
                            </FwbA>
                            <FwbA
                                @click.prevent="openDeleteModal(w)"
                                class="p-1 rounded hover:bg-gray-100"
                            >
