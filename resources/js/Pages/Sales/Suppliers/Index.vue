...
watch(currentPage, (page) => {
    router.get(route("supplier.index"), { page }, { preserveState: true });
});
...
async function submitEdit() {
    ...
    try {
        const response = await axios.put(
            route("supplier.update", selected.value.id),
            formSupplier.value
        );
        toastType.value = "success";
        toastMsg.value =
...
async function submitDelete() {
    ...
    try {
        const response = await axios.delete(
            route("supplier.destroy", selected.value.id)
        );
        toastType.value = "success";
        toastMsg.value =
...
<template>
    <AdminLayout title="Proveedores">
        ...
        <div class="flex justify-between my-6">
            <h2 class="text-2xl font-semibold text-gray-700">Proveedores</h2>
            <FwbButton color="purple" @click="openCreate">
